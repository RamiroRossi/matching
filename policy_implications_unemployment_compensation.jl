# POLICY IMPLICATIONS: ΤU
# UNEMPLOYMENT COMPENSATION AND JOB SEARCH SUBSIDIES
# LOW Q CASE
...
function q_iter!(τv,lab_descount,q,new_ne,iter)
    ...
    model = dmp(;NE = new_ne, τu = τv, A = 10,τw = 6)
...
tu_loop = iter_loop(10,0.99,1,10)
τgrid = collect(-2:0.01:10)
...
name = "y = rVv(τu = $(τgrid[jτ]))") for jτ in eachindex(τgrid)], 
Layout(title="", xaxis_title="Employment"))

plot_rVu = plot([scatter(x= model.E, y=(results_rvu[:,jτ]), 
name = "y = rVu(τu = $(τgrid[jτ]))") for jτ in eachindex(τgrid)], 
Layout(title="", xaxis_title="Employment"))

plot_w = plot([scatter(x= model.E, y=(results_w[:,jτ]), 
name = "y = w(τu = $(τgrid[jτ]))") for jτ in eachindex(τgrid)], 
Layout(title="", xaxis_title="Employment"))
...
self_employment_low_tu = 10 .- (tu_loop[5,:] .+ tu_loop[9,:])
tu_loop[11,:]
employment_τu_low = plot([scatter(x= τgrid, y=tu_loop[5,:], 
name = "E*(τu)")], 
Layout(title="Employment level", xaxis_title="τu"))

unemployment_τu_low = plot([scatter(x= τgrid, y=tu_loop[9,:], 
    name = "U*(τu)")], 
Layout(title="Unemployment level", xaxis_title="τu"));

self_τu_low = plot([scatter(x= τgrid, y=self_employment_low_tu, 
name = "H*(τu)")], 
Layout(title="Self employment level", xaxis_title="τu"));

pop_plot_low_u = ([employment_τu_low unemployment_τu_low self_τu_low])
savefig(pop_plot_low_u,"replication_files/new_figures/unem_compensation/level_V2.png")

# RATES

tot_pop_low_u = self_employment_low_tu .+ tu_loop[5,:] .+ tu_loop[9,:]
tot_pop_low_u = tu_loop[11,:] .+ tu_loop[5,:] .+ tu_loop[9,:]
e_rate_low = tu_loop[5,:] ./ tot_pop_low_u
u_rate_low = tu_loop[9,:] ./ tot_pop_low_u
self_rate_low = tu_loop[11,:] ./ tot_pop_low_u
...
y=self_rate_low, name="H/L")],
 Layout(title = "Employment and Unemployment rates, q = 1",xaxis_title = "τu"))

savefig(plot_rates_low, "replication_files/new_figures/unem_compensation/rates_V2.png")
